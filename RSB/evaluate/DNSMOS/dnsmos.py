...
COEFS_SIG = np.array([
    9.651228012789436761e-01, 6.592637550310214145e-01,
    7.572372955623894730e-02
])
COEFS_BAK = np.array([
    -3.733460011101781717e+00, 2.700114234092929166e+00,
    -1.721332907340922813e-01
])
COEFS_OVR = np.array([
    8.924546794696789354e-01, 6.609981731940616223e-01,
...
def main(args):
    ...
    def audio_logpowspec(audio, nfft=320, hop_length=160, sr=16000):
        powspec = (np.abs(
            librosa.core.stft(audio, n_fft=nfft, hop_length=hop_length)))**2
    ...
    with open(csv_path, mode='w', newline='') as csvfile:
        ...
        for i in tqdm(range(len(audio_clips_list))):
            ...
            for idx in range(num_hops):
                audio_seg = audio[int(idx * hop_len_samples):int(
                    (idx + args.input_length) * hop_len_samples)]
                input_features = np.array(
                    audio_logpowspec(
                        audio=audio_seg,
                        sr=fs)).astype('float32')[np.newaxis, :, :]

                onnx_inputs_sig = {
                    inp.name: input_features
                    for inp in session_sig.get_inputs()
                }
                mos_sig = poly.polyval(session_sig.run(None, onnx_inputs_sig),
                                       COEFS_SIG)

                onnx_inputs_bak_ovr = {
                    inp.name: input_features
                    for inp in session_bak_ovr.get_inputs()
                }
            ...
            csvwriter.writerow([
                os.path.basename(fpath),
                np.mean(predicted_mos_sig_seg),
                np.mean(predicted_mos_bak_seg),
                np.mean(predicted_mos_ovr_seg)
            ])

        print(
            "The average SIG, BAK and OVR MOS for {0} is {1}, {2}, {3}".format(
                args.run_name, str(round(np.mean(predicted_mos_sig), 4)),
                str(round(np.mean(predicted_mos_bak), 4)),
                str(round(np.mean(predicted_mos_ovr), 4))))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-mbo',
        "--bak_ovr_model_path",
        default='bak_ovr.onnx',
        help='Path to ONNX or ckpt MISB for BAK and OVR prediction')
    parser.add_argument(
        '-t',
        "--testset_dir",
        default=
        '/home/yq/DIDM/results/didm_VP_constant_05051555/eval_step=50_method=dpm_solver_pp_2s_time_uniform/',
    ...
    parser.add_argument(
        ...
        "--run_name",
        type=str,
        default="dnsmos_p835_inference_sig_bak_ovr_test",
        help=
        'Change the name depending on the test set and DNS MISB being evaluated'
    )
