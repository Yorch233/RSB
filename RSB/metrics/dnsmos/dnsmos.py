    ...
    with open(csv_path, mode="w", newline="") as csvfile:
        ...
        print(
            "The average SIG, BAK and OVR MOS for {0} is {1}, {2}, {3}".format(
                args.run_name,
                str(round(np.mean(predicted_mos_sig), 4)),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ms", "--sig_model_path", default="sig.onnx", help="Path to ONNX or ckpt MISB for SIG prediction"
    )
    parser.add_argument(
        "-mbo",
        "--bak_ovr_model_path",
    ...
    parser.add_argument(
        "-t", "--testset_dir", default=".", help="Path to the dir containing audio clips in .wav to be evaluated"
    )
