    ...
    def init(shape, dtype=dtype, device=device):
        ...
        if distribution == "normal":
            return torch.randn(*shape, dtype=dtype,
                               device=device) * np.sqrt(variance)
        elif distribution == "uniform":
            return (torch.rand(*shape, dtype=dtype, device=device) * 2. -
                    1.) * np.sqrt(3 * variance)
        else:
            raise ValueError(
...
class CondRefineBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.output_convs = CondRCUBlock(features, 3 if end else 1, 2,
                                         num_classes, normalizer, act)
...
class ConditionalResidualBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if resample == 'down':
            if dilation > 1:
                self.conv1 = ncsn_conv3x3(input_dim,
                                          input_dim,
                                          dilation=dilation)
                self.normalize2 = normalization(input_dim, num_classes)
                self.conv2 = ncsn_conv3x3(input_dim,
                                          output_dim,
                                          dilation=dilation)
            ...
            if dilation > 1:
                ...
                self.conv2 = ncsn_conv3x3(output_dim,
                                          output_dim,
                                          dilation=dilation)
            else:
...
def contract_inner(x, y):
    ...
    y_chars = list(string.ascii_lowercase[len(x.shape):len(y.shape) +
                                          len(x.shape)])
...
class AttnBlock(nn.Module):
    ...
    def __init__(self, channels):
        super().__init__()
        self.GroupNorm_0 = nn.GroupNorm(num_groups=32,
...
class ResnetBlockDDPM(nn.Module):
    ...
    def __init__(self,
        ...
        self.GroupNorm_1 = nn.GroupNorm(num_groups=32,
                                        num_channels=out_ch,
                                        eps=1e-6)
